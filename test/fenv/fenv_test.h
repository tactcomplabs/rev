...
auto float_string( T x ) {
  ...
  case FP_SNAN:
    s << "std::numeric_limits<" << type << ">::signaling_NaN()";
    break;
  case FP_INFINITE:
    if( std::signbit( x ) )
      s << "-";
...
bool test_result( unsigned         test,
                  std::string_view test_src,
                  T                result,
                  T                result_expected,
                  Ts... args ) {
...
template< typename... Ts >
bool test_exceptions( unsigned         test,
                      std::string_view test_src,
                      int              exceptions,
  ...
  if( ( exceptions ^ exceptions_expected ) & FE_ALL_EXCEPT ) {
    ...
    std::cerr << "Expected exceptions: "
              << exception_string( exceptions_expected ) << "\n";
