...
private:
  ...
  EcallStatus ECALL_linkat(RevInst& inst);                 // 37, rev_linkat(int dfd, const char  * pathname, int flag)
  ...
  EcallStatus ECALL_mount(RevInst& inst);                  // 40, rev_mount(char  *name, int flags)
  ...
  EcallStatus ECALL_splice(RevInst& inst);                 // 76, rev_splice(int fd, const struct iovec  *iov, unsigned long nr_segs, unsigned int flags)
