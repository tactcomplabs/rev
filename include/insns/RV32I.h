...
class RV32I : public RevExt {
  ...
  std::vector<RevInstEntry> RV32ITable = {
    ...
    RevInstDefaults().SetMnemonic("andi %rd, %rs1, $imm" ).SetFunct3(   0b111).SetImplFunc(andi  ).Setrs2Class(RevRegClass::RegUNKNOWN).Setimm(FImm)                        .SetFormat(RVTypeI).SetOpcode(0b0010011).SetPredicate( []( uint32_t Inst ) { return DECODE_IMM12( Inst ) != 0xff; } ),
    RevInstDefaults().SetMnemonic("zext.b %rd, %rs"      ).SetFunct3(   0b111).SetImplFunc(andi  ).Setrs2Class(RevRegClass::RegUNKNOWN).Setimm(FImm)                        .SetFormat(RVTypeI).SetOpcode(0b0010011).SetPredicate( []( uint32_t Inst ) { return DECODE_IMM12( Inst ) == 0xff; } ),
