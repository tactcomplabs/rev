...
public:
  ...
  void SetParentID( const uint32_t NewParentID ) {
    ParentID = NewParentID;
  }

  ///< RevThread: Get the TID of the thread that this thread is waiting to join
  uint32_t GetWaitingToJoinTID() const {
    return WaitingToJoinTID;
  }
  ...
  bool FindFD( int fd ) {
    return fildes.count( fd );
  }

  ///< RevThread: Get the fildes valid for this thread
  const std::unordered_set< int >& GetFildes() {
    return fildes;
  }
  ...
  void AddChildID( uint32_t tid ) {
    ChildrenIDs.insert( tid );
  }

  ///< RevThread: Remove a child thread ID from this thread
  void RemoveChildID( uint32_t tid ) {
    ChildrenIDs.erase( tid );
  }
