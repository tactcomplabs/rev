...
RevHeap::RevHeap( RevMem* Mem, const size_t MaxHeapSize, const uint64_t baseAddr, SST::Output *output)
  : Mem(Mem), Size(0), MaxSize(MaxHeapSize), BaseAddr(baseAddr), output(output){
  ...
  if( BaseAddr % 64 != 0 ){
    output->fatal(CALL_INFO, -1, "RevHeap: BaseAddr must be a multiple of 64B\n");
  }

  if( MaxHeapSize < 64 ){
    output->fatal(CALL_INFO, -1, "RevHeap: MaxHeapSize must be at least 64B\n");
  }

  output->verbose(CALL_INFO, 1, 0, "RevHeap: Initialized with MaxHeapSize=%zu "
                                   "and BaseAddr=%" PRIx64 "\n", MaxHeapSize, BaseAddr);
...
void RevHeap::sbrk(int64_t increment){
  if( increment < 0 ){
    if( Size + increment < 0 ){
      output->fatal(CALL_INFO, -1, "RevHeap: SBRK Increment Value would result in a negative heap size\n");
    }
