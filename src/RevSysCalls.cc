...
EcallStatus RevProc::ECALL_io_setup(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_setup called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 1, rev_io_destroy(aio_context_t ctx)
EcallStatus RevProc::ECALL_io_destroy(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_destroy called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 2, rev_io_submit(aio_context_t, long, struct iocb  *  *)
EcallStatus RevProc::ECALL_io_submit(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_submit called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_io_cancel(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_cancel called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_io_getevents(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_getevents called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_lsetxattr(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: lsetxattr called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_lookup_dcookie(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: lookup_dcookie called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 19, rev_eventfd2(unsigned int count, int flags)
EcallStatus RevProc::ECALL_eventfd2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: eventfd2 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 20, rev_epoll_create1(int flags)
EcallStatus RevProc::ECALL_epoll_create1(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: epoll_create1 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 21, rev_epoll_ctl(int epfd, int op, int fd, struct epoll_event  *event)
EcallStatus RevProc::ECALL_epoll_ctl(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: epoll_ctl called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_unlinkat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: unlinkat called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 36, rev_symlinkat(const char  * oldname, int newdfd, const char  * newname)
EcallStatus RevProc::ECALL_symlinkat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: symlinkat called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 37, rev_unlinkat(int dfd, const char  * pathname, int flag)
EcallStatus RevProc::ECALL_linkat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: linkat called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_fchdir(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fchdir called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 51, rev_chroot(const char  *filename)
EcallStatus RevProc::ECALL_chroot(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: chroot called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 52, rev_fchmod(unsigned int fd, umode_t mode)
EcallStatus RevProc::ECALL_fchmod(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fchmod called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 53, rev_fchmodat(int dfd, const char  * filename, umode_t mode)
EcallStatus RevProc::ECALL_fchmodat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fchmodat called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_fchownat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fchownat called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 55, rev_fchown(unsigned int fd, uid_t user, gid_t group)
EcallStatus RevProc::ECALL_fchown(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fchown called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_openat(RevInst& inst){
  ...
  auto action = [&]{
    ...
    ThreadOnHart(HartToExec)->AddFD(fd);
...
EcallStatus RevProc::ECALL_vhangup(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: vhangup called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 59, rev_pipe2(int  *fildes, int flags)
EcallStatus RevProc::ECALL_pipe2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pipe2 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 60, rev_quotactl(unsigned int cmd, const char  *special, qid_t id, void  *addr)
EcallStatus RevProc::ECALL_quotactl(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: quotactl called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_getdents64(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getdents64 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_lseek(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: lseek called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_read(RevInst& inst){
  ...
  if( !ActiveThread->FindFD(fd) ){
    output->fatal(CALL_INFO, -1,
                  ...
                  " tried to read from file descriptor: %" PRIi32 ", but did not have access to it\n",
                  id, HartToExec, GetActiveThreadID(), fd);
    return EcallStatus::SUCCESS;
  }
  /*
...
EcallStatus RevProc::ECALL_write(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: write called by thread %" PRIu16 " on hart %" PRIu32 "\n", GetActiveThreadID(), HartToExec);
  auto fd = RegFile->GetX<int>(RevReg::a0);
...
EcallStatus RevProc::ECALL_preadv(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: preadv called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_pwritev(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pwritev called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 71, rev_sendfile64(int out_fd, int in_fd, loff_t  *offset, size_t count)
EcallStatus RevProc::ECALL_sendfile64(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sendfile64 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_pselect6_time32(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pselect6_time32 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_ppoll_time32(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: ppoll_time32 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 74, rev_signalfd4(int ufd, sigset_t  *user_mask, size_t sizemask, int flags)
EcallStatus RevProc::ECALL_signalfd4(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: signalfd4 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_vmsplice(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: vmsplice called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_splice(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: splice called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 77, rev_tee(int fdin, int fdout, size_t len, unsigned int flags)
EcallStatus RevProc::ECALL_tee(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: tee called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 78, rev_readlinkat(int dfd, const char  *path, char  *buf, int bufsiz)
EcallStatus RevProc::ECALL_readlinkat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: readlinkat called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_newfstatat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: newfstatat called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 80, rev_newfstat(unsigned int fd, struct stat  *statbuf)
EcallStatus RevProc::ECALL_newfstat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: newfstat called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 81, rev_sync(void)
EcallStatus RevProc::ECALL_sync(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sync called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 82, rev_fsync(unsigned int fd)
EcallStatus RevProc::ECALL_fsync(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fsync called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 83, rev_fdatasync(unsigned int fd)
EcallStatus RevProc::ECALL_fdatasync(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fdatasync called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_sync_file_range2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sync_file_range2 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_sync_file_range(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sync_file_range called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 85, rev_timerfd_create(int clockid, int flags)
EcallStatus RevProc::ECALL_timerfd_create(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timerfd_create called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_timerfd_settime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timerfd_settime called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 87, rev_timerfd_gettime(int ufd, struct __kernel_itimerspec  *otmr)
EcallStatus RevProc::ECALL_timerfd_gettime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timerfd_gettime called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_utimensat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: utimensat called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 89, rev_acct(const char  *name)
EcallStatus RevProc::ECALL_acct(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: acct called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 90, rev_capget(cap_user_header_t header, cap_user_data_t dataptr)
EcallStatus RevProc::ECALL_capget(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: capget called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 91, rev_capset(cap_user_header_t header, const cap_user_data_t data)
EcallStatus RevProc::ECALL_capset(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: capset called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 92, rev_personality(unsigned int personality)
EcallStatus RevProc::ECALL_personality(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: personality called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_exit_group(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: exit_group called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_waitid(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: waitid called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 96, rev_set_tid_address(int  *tidptr)
EcallStatus RevProc::ECALL_set_tid_address(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: set_tid_address called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 97, rev_unshare(unsigned long unshare_flags)
EcallStatus RevProc::ECALL_unshare(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: unshare called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_futex(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: futex called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 99, rev_set_robust_list(struct robust_list_head  *head, size_t len)
EcallStatus RevProc::ECALL_set_robust_list(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: set_robust_list called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_get_robust_list(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: get_robust_list called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_nanosleep(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: nanosleep called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 102, rev_getitimer(int which, struct __kernel_old_itimerval  *value)
EcallStatus RevProc::ECALL_getitimer(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getitimer called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_setitimer(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: setitimer called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_kexec_load(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: kexec_load called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 105, rev_init_module(void  *umod, unsigned long len, const char  *uargs)
EcallStatus RevProc::ECALL_init_module(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: init_module called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 106, rev_delete_module(const char  *name_user, unsigned int flags)
EcallStatus RevProc::ECALL_delete_module(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: delete_module called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_timer_create(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timer_create called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_timer_gettime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timer_gettime called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 109, rev_timer_getoverrun(timer_t timer_id)
EcallStatus RevProc::ECALL_timer_getoverrun(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timer_getoverrun called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_timer_settime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timer_settime called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 111, rev_timer_delete(timer_t timer_id)
EcallStatus RevProc::ECALL_timer_delete(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timer_delete called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_clock_settime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clock_settime called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_clock_gettime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clock_gettime called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_clock_getres(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clock_getres called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_clock_nanosleep(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clock_nanosleep called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 116, rev_syslog(int type, char  *buf, int len)
EcallStatus RevProc::ECALL_syslog(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: syslog called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 117, rev_ptrace(long request, long pid, unsigned long addr, unsigned long data)
EcallStatus RevProc::ECALL_ptrace(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: ptrace called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 118, rev_sched_setparam(pid_t pid, struct sched_param  *param)
EcallStatus RevProc::ECALL_sched_setparam(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_setparam called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_sched_setscheduler(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_setscheduler called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 120, rev_sched_getscheduler(pid_t pid)
EcallStatus RevProc::ECALL_sched_getscheduler(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_getscheduler called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 121, rev_sched_getparam(pid_t pid, struct sched_param  *param)
EcallStatus RevProc::ECALL_sched_getparam(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_getparam called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_sched_setaffinity(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_setaffinity called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_sched_getaffinity(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_getaffinity called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 124, rev_sched_yield(void)
EcallStatus RevProc::ECALL_sched_yield(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_yield called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 125, rev_sched_get_priority_max(int policy)
EcallStatus RevProc::ECALL_sched_get_priority_max(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_get_priority_max called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 126, rev_sched_get_priority_min(int policy)
EcallStatus RevProc::ECALL_sched_get_priority_min(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_get_priority_min called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_sched_rr_get_interval(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_rr_get_interval called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 128, rev_restart_syscall(void)
EcallStatus RevProc::ECALL_restart_syscall(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: restart_syscall called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 129, rev_kill(pid_t pid, int sig)
EcallStatus RevProc::ECALL_kill(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: kill called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 130, rev_tkill(pid_t pid, int sig)
EcallStatus RevProc::ECALL_tkill(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: tkill called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 131, rev_tgkill(pid_t tgid, pid_t pid, int sig)
EcallStatus RevProc::ECALL_tgkill(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: tgkill called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_sigaltstack(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sigaltstack called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 133, rev_rt_sigsuspend(sigset_t  *unewset, size_t sigsetsize)
EcallStatus RevProc::ECALL_rt_sigsuspend(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rt_sigsuspend called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_rt_sigaction(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rt_sigaction called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_rt_sigprocmask(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rt_sigprocmask called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 136, rev_rt_sigpending(sigset_t  *set, size_t sigsetsize)
EcallStatus RevProc::ECALL_rt_sigpending(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rt_sigpending called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_prctl(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: prctl called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 168, rev_getcpu(unsigned  *cpu, unsigned  *node, struct getcpu_cache  *cache)
EcallStatus RevProc::ECALL_getcpu(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getcpu called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_gettimeofday(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: gettimeofday called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_settimeofday(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: settimeofday called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 171, rev_adjtimex(struct __kernel_timex  *txc_p)
EcallStatus RevProc::ECALL_adjtimex(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: adjtimex called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_getuid(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getuid called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 175, rev_geteuid(void)
EcallStatus RevProc::ECALL_geteuid(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: geteuid called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 176, rev_getgid(void)
EcallStatus RevProc::ECALL_getgid(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getgid called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 177, rev_getegid(void)
EcallStatus RevProc::ECALL_getegid(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getegid called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 178, rev_gettid(void)
EcallStatus RevProc::ECALL_gettid(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: gettid called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
...
EcallStatus RevProc::ECALL_sysinfo(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sysinfo called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 180, rev_mq_open(const char  *name, int oflag, umode_t mode, struct mq_attr  *attr)
EcallStatus RevProc::ECALL_mq_open(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mq_open called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 181, rev_mq_unlink(const char  *name)
EcallStatus RevProc::ECALL_mq_unlink(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mq_unlink called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_mq_timedsend(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mq_timedsend called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_mq_timedreceive(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mq_timedreceive called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 184, rev_mq_notify(mqd_t mqdes, const struct sigevent  *notification)
EcallStatus RevProc::ECALL_mq_notify(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mq_notify called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_mq_getsetattr(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mq_getsetattr called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 186, rev_msgget(key_t key, int msgflg)
EcallStatus RevProc::ECALL_msgget(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: msgget called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 187, rev_old_msgctl(int msqid, int cmd, struct msqid_ds  *buf)
EcallStatus RevProc::ECALL_msgctl(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: msgctl called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_msgrcv(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: msgrcv called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 189, rev_msgsnd(int msqid, struct msgbuf  *msgp, size_t msgsz, int msgflg)
EcallStatus RevProc::ECALL_msgsnd(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: msgsnd called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 190, rev_semget(key_t key, int nsems, int semflg)
EcallStatus RevProc::ECALL_semget(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: semget called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 191, rev_semctl(int semid, int semnum, int cmd, unsigned long arg)
EcallStatus RevProc::ECALL_semctl(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: semctl called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_brk(RevInst& inst){
  ...
  if( Addr > 0 && Addr > heapend ){
    ...
    output->fatal(CALL_INFO, 11,
                  "Out of memory / Unable to expand system break (brk) to Addr = 0x%" PRIx64, Addr);
  }
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_munmap(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: munmap called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  auto Addr = RegFile->GetX<uint64_t>(RevReg::a0);
  auto Size = RegFile->GetX<uint64_t>(RevReg::a1);
  ...
  if(rc == -1){
    output->fatal(CALL_INFO, 11,
                  "Failed to perform munmap(Addr = 0x%" PRIx64 ", Size = 0x%" PRIu64 ")"
...
EcallStatus RevProc::ECALL_mremap(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mremap called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_add_key(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: add_key called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_request_key(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: request_key called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_keyctl(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: keyctl called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_execve(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: execve called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 222, rev_old_mmap(struct mmap_arg_struct  *arg)
EcallStatus RevProc::ECALL_mmap(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mmap called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
...
EcallStatus RevProc::ECALL_remap_file_pages(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: remap_file_pages called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_mbind(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mbind called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_get_mempolicy(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: get_mempolicy called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_set_mempolicy(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: set_mempolicy called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_migrate_pages(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: migrate_pages called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_move_pages(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: move_pages called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_rt_tgsigqueueinfo(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rt_tgsigqueueinfo called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 241, rev_perf_event_open(")
EcallStatus RevProc::ECALL_perf_event_open(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: perf_event_open called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 242, rev_accept4(int, struct sockaddr  *, int  *, int)
EcallStatus RevProc::ECALL_accept4(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: accept4 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_recvmmsg_time32(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: recvmmsg_time32 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 260, rev_wait4(pid_t pid, int  *stat_addr, int options, struct rusage  *ru)
EcallStatus RevProc::ECALL_wait4(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: wait4 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_prlimit64(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: prlimit64 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 262, rev_fanotify_init(unsigned int flags, unsigned int event_f_flags)
EcallStatus RevProc::ECALL_fanotify_init(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fanotify_init called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_fanotify_mark(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fanotify_mark called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_name_to_handle_at(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: name_to_handle_at called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_open_by_handle_at(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: open_by_handle_at called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 266, rev_clock_adjtime(clockid_t which_clock, struct __kernel_timex  *tx)
EcallStatus RevProc::ECALL_clock_adjtime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clock_adjtime called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 267, rev_syncfs(int fd)
EcallStatus RevProc::ECALL_syncfs(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: syncfs called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 268, rev_setns(int fd, int nstype)
EcallStatus RevProc::ECALL_setns(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: setns called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_sendmmsg(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sendmmsg called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_process_vm_readv(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: process_vm_readv called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_process_vm_writev(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: process_vm_writev called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
...
EcallStatus RevProc::ECALL_kcmp(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: kcmp called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 273, rev_finit_module(int fd, const char  *uargs, int flags)
EcallStatus RevProc::ECALL_finit_module(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: finit_module called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_sched_setattr(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_setattr called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_sched_getattr(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_getattr called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_execveat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: execveat called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 282, rev_userfaultfd(int flags)
EcallStatus RevProc::ECALL_userfaultfd(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: userfaultfd called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 283, rev_membarrier(int cmd, unsigned int flags, int cpu_id)
EcallStatus RevProc::ECALL_membarrier(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: membarrier called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 284, rev_mlock2(unsigned long start, size_t len, int flags)
EcallStatus RevProc::ECALL_mlock2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mlock2 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_copy_file_range(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: copy_file_range called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_preadv2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: preadv2 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_pwritev2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pwritev2 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_pkey_mprotect(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pkey_mprotect called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 289, rev_pkey_alloc(unsigned long flags, unsigned long init_val)
EcallStatus RevProc::ECALL_pkey_alloc(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pkey_alloc called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 290, rev_pkey_free(int pkey)
EcallStatus RevProc::ECALL_pkey_free(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pkey_free called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_statx(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: statx called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_io_pgetevents(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_pgetevents called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 293, rev_rseq(struct rseq  *rseq, uint32_t rseq_len, int flags, uint32_t sig)
EcallStatus RevProc::ECALL_rseq(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rseq called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_kexec_file_load(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: kexec_file_load called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_pidfd_send_signal(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pidfd_send_signal called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 425, rev_io_uring_setup(u32 entries, struct io_uring_params  *p)
EcallStatus RevProc::ECALL_io_uring_setup(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_uring_setup called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_io_uring_enter(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_uring_enter called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_io_uring_register(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_uring_register called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 428, rev_open_tree(int dfd, const char  *path, unsigned flags)
EcallStatus RevProc::ECALL_open_tree(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: open_tree called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_move_mount(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: move_mount called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 430, rev_fsopen(const char  *fs_name, unsigned int flags)
EcallStatus RevProc::ECALL_fsopen(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fsopen called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_fsconfig(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fsconfig called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 432, rev_fsmount(int fs_fd, unsigned int flags, unsigned int ms_flags)
EcallStatus RevProc::ECALL_fsmount(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fsmount called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 433, rev_fspick(int dfd, const char  *path, unsigned int flags)
EcallStatus RevProc::ECALL_fspick(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fspick called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 434, rev_pidfd_open(pid_t pid, unsigned int flags)
EcallStatus RevProc::ECALL_pidfd_open(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pidfd_open called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 435, rev_clone3(struct clone_args  *uargs, size_t size)
EcallStatus RevProc::ECALL_clone3(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clone3 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  auto rtval = EcallStatus::SUCCESS;
 //  auto CloneArgsAddr = RegFile->GetX<uint64_t>(RevReg::a0);
 // auto SizeOfCloneArgs = RegFile()->GetX<size_t>(RevReg::a1);
...
EcallStatus RevProc::ECALL_close_range(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: close_range called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_openat2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: openat2 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 438, rev_pidfd_getfd(int pidfd, int fd, unsigned int flags)
EcallStatus RevProc::ECALL_pidfd_getfd(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pidfd_getfd called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}

// 439, rev_faccessat2(int dfd, const char  *filename, int mode, int flags)
EcallStatus RevProc::ECALL_faccessat2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: faccessat2 called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_process_madvise(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: process_madvise called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  return EcallStatus::SUCCESS;
}
...
EcallStatus RevProc::ECALL_pthread_create(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pthread_create called by thread %" PRIu32 "on hart %" PRIu16 "\n", GetActiveThreadID(), HartToExec);
  uint64_t tidAddr     = RegFile->GetX<uint64_t>(RevReg::a0);
  // uint64_t AttrPtr     = RegFile->GetX<uint64_t>(RevReg::a1); // NOT USED
...
EcallStatus RevProc::ECALL_pthread_join(RevInst& inst){
  ...
  // Make sure this thread doesn't have any dependencies... if it doesn't, make it BLOCKED
  if( ThreadCanBeRemoved(Harts.at(inst.hart)->GetAssignedThreadID() ) ) {
    rtval = EcallStatus::SUCCESS;

    // Set current thread to blocked
    auto& Thread = ThreadOnHart(HartToExec);
