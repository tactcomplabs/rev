...
RevProc::ECALL_status_t RevProc::ECALL_io_destroy(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_destroy called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 2, rev_io_submit(aio_context_t, long, struct iocb  *  *)
RevProc::ECALL_status_t RevProc::ECALL_io_submit(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_submit called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_io_cancel(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_cancel called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_io_getevents(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_getevents called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_lsetxattr(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: lsetxattr called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_fsetxattr(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fsetxattr called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_getxattr(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getxattr called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_lgetxattr(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: lgetxattr called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_getcwd(RevInst& inst){
  uint64_t BufAddr = RegFile->GetX<uint64_t>(feature, 10);
  uint64_t size = RegFile->GetX<uint64_t>(feature, 11);
...
// 17, rev_getcwd(char  *buf, unsigned long size)
RevProc::ECALL_status_t RevProc::ECALL_getcwd(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getcwd called by thread %i\n", GetActiveThreadID());
  uint64_t BufAddr = RegFile->GetX<uint64_t>(feature, 10);
  uint64_t size = RegFile->GetX<uint64_t>(feature, 11);
  std::string CWD = std::filesystem::current_path();
...
RevProc::ECALL_status_t RevProc::ECALL_lookup_dcookie(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: lookup_dcookie called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 19, rev_eventfd2(unsigned int count, int flags)
RevProc::ECALL_status_t RevProc::ECALL_eventfd2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: eventfd2 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 20, rev_epoll_create1(int flags)
RevProc::ECALL_status_t RevProc::ECALL_epoll_create1(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: epoll_create1 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_epoll_ctl(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: epoll_ctl called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_fcntl64(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fcntl64 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 26, rev_inotify_init1(int flags)
RevProc::ECALL_status_t RevProc::ECALL_inotify_init1(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: inotify_init1 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_mknodat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mknodat called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_mkdirat(RevInst& inst){
  ...
  int      dirfd    = RegFile->GetX<int>(feature, 10);
  uint64_t pathAddr = RegFile->GetX<uint64_t>(feature, 11);
  mode_t   mode     = RegFile->GetX<mode_t>(feature, 12);
  ...
  return ECALL_LoadAndParseString(inst, pathAddr, action);
}
...
RevProc::ECALL_status_t RevProc::ECALL_mkdirat(RevInst& inst){
  ...
  if('\0' != ECALL.buf[0]){
    ...
    mem->ReadVal<char>(HartToExec, RegFile->RV64[11] + sizeof(char)*ECALL.string.length(), &ECALL.buf[0], inst.hazard, REVMEM_FLAGS(0x00));
    ...
    /* Do the mkdirat on the host */
    const int rc = mkdirat(fd, ECALL.string.data(), Mode);
    RegFile->RV64[10] = rc;
    ...
    mem->ReadVal<char>(HartToExec, RegFile->RV64[11], &ECALL.buf[0], inst.hazard, REVMEM_FLAGS(0x00));
    DependencySet(HartToExec, 10, false);
    rtval = ECALL_status_t::CONTINUE;
  }
/*
...
RevProc::ECALL_status_t RevProc::ECALL_unlinkat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: unlinkat called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_symlinkat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: symlinkat called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 37, rev_unlinkat(int dfd, const char  * pathname, int flag)
RevProc::ECALL_status_t RevProc::ECALL_linkat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: linkat called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_chdir(RevInst& inst){
 ...
 RevProc::ECALL_status_t rtval = ECALL_status_t::SUCCESS;
  ...
  if('\0' != ECALL.buf[0]){
    //We are in the middle of the string
    ECALL.string = ECALL.string + ECALL.buf[0];
    mem->ReadVal<char>(HartToExec, RegFile->RV64[10] + sizeof(char)*ECALL.string.length(), &ECALL.buf[0], inst.hazard, REVMEM_FLAGS(0x00));
    rtval = ECALL_status_t::CONTINUE;
  }else if(('\0' == ECALL.buf[0]) && (ECALL.string.length() > 0)) {
    //found the null terminator - we're done
    ECALL.string = ECALL.string + ECALL.buf[0];
    const int rc = chdir(ECALL.string.data());
    RegFile->RV64[10] = rc;
    ECALL.string.clear();   //reset the ECALL buffers
    ECALL.buf[0] = '\0';
    DependencyClear(HartToExec, 10, false);
    ...
    //first time through the ECALL
    mem->ReadVal<char>(HartToExec, RegFile->RV64[10], &ECALL.buf[0], inst.hazard, REVMEM_FLAGS(0x00));
    rtval = ECALL_status_t::CONTINUE;
    DependencySet(HartToExec, 10, false);
  }
...
RevProc::ECALL_status_t RevProc::ECALL_fchownat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fchownat called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 55, rev_fchown(unsigned int fd, uid_t user, gid_t group)
RevProc::ECALL_status_t RevProc::ECALL_fchown(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fchown called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

#if USE_OLD_SYSCALLS || 1

// 56, rev_openat(int dirfd, const char  *filename, int flags, umode_t mode)
RevProc::ECALL_status_t RevProc::ECALL_openat(RevInst& inst){
  int dirfd = RegFile->GetX<int>(feature, 10);
  int filenameAddr = RegFile->GetX<int>(feature, 11);

  // commented out to remove warnings
  // int flags = RegFile->GetX<int>(feature, 12); /* NOTE: Unused for now */
  // umode_t mode = RegFile->GetX<umode_t>(feature, 13);
  ...
  */
  ...
  auto action = [&]{
    // TODO: Fix kludge which uses current working directory
...
RevProc::ECALL_status_t RevProc::ECALL_close(RevInst& inst){
  int fd = RegFile->GetX<int>(feature, 10);
  auto CurrThread = AssignedThreads.at(HartToDecode);


  // Check if CurrThread has fd in fildes vector
  if( !CurrThread->FindFD(fd) ){
    output->fatal(CALL_INFO, -1,
                  "Core %d; Hart %d; ThreadID %" PRIu32 " tried to close file"
                  " descriptor %d but did not have access to it\n",
  ...
  /* Remove from Thread's fildes */
  CurrThread->RemoveFD(fd);
...
RevProc::ECALL_status_t RevProc::ECALL_vhangup(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: vhangup called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 59, rev_pipe2(int  *fildes, int flags)
RevProc::ECALL_status_t RevProc::ECALL_pipe2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pipe2 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_quotactl(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: quotactl called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_getdents64(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getdents64 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_lseek(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: lseek called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_read(RevInst& inst){
  ...
  if( !CurrThread->FindFD(fd) ){
    output->fatal(CALL_INFO, -1,
                  "Core %d; Hart %d; ThreadID %" PRIu32
                  " tried to read from file descriptor: %i"
                  " but did not have access to it\n",
                  id, HartToExec, GetActiveThreadID(), fd);
    return ECALL_status_t::SUCCESS;
  }

  // This buffer is an intermediate buffer for storing the data read from host for
  // later use in writing to RevMem. Note: Variable-length Arrays are non-portable
...
RevProc::ECALL_status_t RevProc::ECALL_read(RevInst& inst){
  ...
  // This buffer is an intermediate buffer for storing the data read from host for
  // later use in writing to RevMem. Note: Variable Length Arrays are not portable
  std::vector<char> TmpBuf(BufSize);
...
RevProc::ECALL_status_t RevProc::ECALL_write(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: write called by thread %i\n", GetActiveThreadID());
  auto rtv = ECALL_status_t::ERROR;

  int fd = RegFile->GetX<int>(feature, 10);
  uint64_t addr = RegFile->GetX<uint64_t>(feature, 11);
  size_t nbytes = RegFile->GetX<size_t>(feature, 12);

  // std::string can have embedded 0 bytes and has a size() and data()
  // which is not necessarily null-terminated strings. For binary
  // operations like read()/write(), we should treat data as binary.
  if(ECALL.bytesRead){
  ...
  if(nleft == 0){
    ...
    // Choose the largest-sized word which is not greater than nleft
    // and write than many bytes
    if(nleft >= 8){
...
RevProc::ECALL_status_t RevProc::ECALL_write(RevInst& inst){
  ...
  if(ECALL.bytesRead == nbytes){
    // Read is complete - send to host
    ECALL.string = ECALL.string.append(ECALL.buf);
    // Perform the write on the host system
    const int rc = write(fildes, ECALL.string.data(), nbytes);
    ...
    if(1 == (nbytes - ECALL.bytesRead)){
      mem->ReadVal<char>(HartToExec, RegFile->RV64[11] + ECALL.bytesRead, ECALL.buf, inst.hazard, REVMEM_FLAGS(0x0));
      ECALL.buf[1] = '\0';  //Pre-null terminate to allow for succesful string cat later
      ECALL.bytesRead += sizeof(char);
    }
    else if(3 >= (nbytes - ECALL.bytesRead )){
      mem->ReadVal<int16_t>(HartToExec, RegFile->RV64[11] + ECALL.bytesRead, reinterpret_cast<int16_t*>(ECALL.buf), inst.hazard, REVMEM_FLAGS(0x0));
      ECALL.buf[2] = '\0'; //Pre-null terminate to allow for succesful string cat later
      ECALL.bytesRead += sizeof(int16_t);
    }
    else if(7 >= (nbytes - ECALL.bytesRead )){
      mem->ReadVal<int32_t>(HartToExec, RegFile->RV64[11] + ECALL.bytesRead, reinterpret_cast<int32_t*>(ECALL.buf), inst.hazard, REVMEM_FLAGS(0x0));
      ECALL.buf[4] = '\0'; //Pre-null terminate to allow for succesful string cat later
      ECALL.bytesRead += sizeof(int32_t);
    }
    else {
      mem->ReadVal<int64_t>(HartToExec, RegFile->RV64[11] + ECALL.bytesRead, reinterpret_cast<int64_t*>(ECALL.buf), inst.hazard, REVMEM_FLAGS(0x0));
      ECALL.buf[8] = '\0'; //Pre-null terminate to allow for succesful string cat later
     ECALL.bytesRead += sizeof(int64_t);
    }
    rtv = ECALL_status_t::CONTINUE;
    DependencySet(HartToExec, 10, false);
  }
...
RevProc::ECALL_status_t RevProc::ECALL_readv(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: readv called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_writev(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: writev called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 67, rev_pread64(unsigned int fd, char  *buf, size_t count, loff_t pos)
RevProc::ECALL_status_t RevProc::ECALL_pread64(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pread64 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_pwrite64(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pwrite64 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_preadv(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: preadv called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_pwritev(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pwritev called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_sendfile64(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sendfile64 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_pselect6_time32(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pselect6_time32 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_ppoll_time32(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: ppoll_time32 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_signalfd4(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: signalfd4 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_vmsplice(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: vmsplice called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_splice(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: splice called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 77, rev_tee(int fdin, int fdout, size_t len, unsigned int flags)
RevProc::ECALL_status_t RevProc::ECALL_tee(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: tee called by thread %i\n", GetActiveThreadID());
#if 0
  // commented out to remove warnings
  int fd_in      = RegFile->GetX<int>(feature, 10);
  int fd_out     = RegFile->GetX<int>(feature, 11);
  size_t len     = RegFile->GetX<size_t>(feature, 12);
  unsigned flags = RegFile->GetX<unsigned>(feature, 13);
#endif
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_readlinkat(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: readlinkat called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_sync_file_range2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sync_file_range2 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_sync_file_range(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sync_file_range called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 85, rev_timerfd_create(int clockid, int flags)
RevProc::ECALL_status_t RevProc::ECALL_timerfd_create(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timerfd_create called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_timerfd_settime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timerfd_settime called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_timerfd_gettime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timerfd_gettime called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_capset(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: capset called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 92, rev_personality(unsigned int personality)
RevProc::ECALL_status_t RevProc::ECALL_personality(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: personality called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_exit(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: exit called by thread %i\n", GetActiveThreadID());
  int status = RegFile->GetX<int>(feature, 10);

  // Check if Current Thread has access to the fd
  auto CurrThread = AssignedThreads.at(HartToDecode);

  // If the current ctx has ParentPID = 0,
  // it has no parent and we should terminate the simulator
  if( CurrThread->GetParentThreadID() == 0 ){
    output->verbose(CALL_INFO, 0, 0,
                    "Process %u exiting with status %d\n",
                    CurrThread->GetThreadID(), status );
    exit(status);
  } else {
    // Parent exists & Child is exiting... switch back to parent
    // ThreadSwitchAlert(CurrThread->GetParentThreadID());
    output->verbose(CALL_INFO, 0, 0,
                    "Process %u exiting with status %d\n",
                    CurrThread->GetThreadID(), status );
  }
...
RevProc::ECALL_status_t RevProc::ECALL_exit_group(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: exit_group called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_futex(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: futex called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_set_robust_list(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: set_robust_list called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_get_robust_list(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: get_robust_list called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_nanosleep(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: nanosleep called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_getitimer(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getitimer called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_setitimer(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: setitimer called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_kexec_load(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: kexec_load called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_init_module(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: init_module called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_delete_module(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: delete_module called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_timer_create(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timer_create called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_timer_gettime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timer_gettime called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 109, rev_timer_getoverrun(timer_t timer_id)
RevProc::ECALL_status_t RevProc::ECALL_timer_getoverrun(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timer_getoverrun called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_timer_settime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timer_settime called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 111, rev_timer_delete(timer_t timer_id)
RevProc::ECALL_status_t RevProc::ECALL_timer_delete(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: timer_delete called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_clock_settime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clock_settime called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_clock_gettime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clock_gettime called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_clock_getres(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clock_getres called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_clock_nanosleep(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clock_nanosleep called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 116, rev_syslog(int type, char  *buf, int len)
RevProc::ECALL_status_t RevProc::ECALL_syslog(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: syslog called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_ptrace(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: ptrace called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_sched_setparam(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_setparam called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_sched_setscheduler(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_setscheduler called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 120, rev_sched_getscheduler(pid_t pid)
RevProc::ECALL_status_t RevProc::ECALL_sched_getscheduler(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_getscheduler called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_sched_getparam(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_getparam called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_sched_setaffinity(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_setaffinity called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_sigaltstack(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sigaltstack called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 133, rev_rt_sigsuspend(sigset_t  *unewset, size_t sigsetsize)
RevProc::ECALL_status_t RevProc::ECALL_rt_sigsuspend(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rt_sigsuspend called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_rt_sigaction(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rt_sigaction called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_rt_sigprocmask(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rt_sigprocmask called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 136, rev_rt_sigpending(sigset_t  *set, size_t sigsetsize)
RevProc::ECALL_status_t RevProc::ECALL_rt_sigpending(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rt_sigpending called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_rt_sigtimedwait_time32(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rt_sigtimedwait_time32 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_prctl(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: prctl called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_getcpu(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getcpu called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_gettimeofday(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: gettimeofday called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_settimeofday(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: settimeofday called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 171, rev_adjtimex(struct __kernel_timex  *txc_p)
RevProc::ECALL_status_t RevProc::ECALL_adjtimex(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: adjtimex called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_getpid(RevInst& inst){
  ...
  output->verbose(CALL_INFO, 2, 0, "ECALL: getpid called by thread %i\n", GetActiveThreadID());
...
#ifdef USE_OLD_SYSCALLS

// 173, rev_getppid(void)
RevProc::ECALL_status_t RevProc::ECALL_getppid(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getppid called by thread %i\n", GetActiveThreadID());
...
RevProc::ECALL_status_t RevProc::ECALL_getppid(RevInst& inst){
  ...
  // uint32_t CurrentThreadID = ActiveThreadIDs.at(HartToExec);
  // auto CurrentThread = ThreadTable.at(CurrentThreadID);
  // uint32_t ParentThreadID = CurrentThread->GetParentThreadID();
  // RegFile->SetX(feature, 10, ParentPID);
...
RevProc::ECALL_status_t RevProc::ECALL_getuid(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getuid called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 175, rev_geteuid(void)
RevProc::ECALL_status_t RevProc::ECALL_geteuid(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: geteuid called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 176, rev_getgid(void)
RevProc::ECALL_status_t RevProc::ECALL_getgid(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getgid called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 177, rev_getegid(void)
RevProc::ECALL_status_t RevProc::ECALL_getegid(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: getegid called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_gettid(RevInst& inst){
  ...
  RegFile->SetX(feature, 10, AssignedThreads.at(HartToExec)->GetThreadID());
  return ECALL_status_t::SUCCESS;
}

// 179, rev_sysinfo(struct sysinfo  *info)
RevProc::ECALL_status_t RevProc::ECALL_sysinfo(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sysinfo called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_mq_open(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mq_open called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 181, rev_mq_unlink(const char  *name)
RevProc::ECALL_status_t RevProc::ECALL_mq_unlink(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mq_unlink called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_mq_timedsend(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mq_timedsend called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_mq_timedreceive(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mq_timedreceive called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_mq_notify(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mq_notify called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_msgrcv(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: msgrcv called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_setsockopt(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: setsockopt called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_brk(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: brk called by thread %i\n", GetActiveThreadID());
  int64_t Addr = RegFile->GetX<uint64_t>(feature, 10);
...
RevProc::ECALL_status_t RevProc::ECALL_brk(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: brk called by thread %i\n", GetActiveThreadID());
  uint64_t Addr = RegFile->GetX<uint64_t>(feature, 10);
...
// 215, rev_munmap(uintptr_t addr, size_t len)
RevProc::ECALL_status_t RevProc::ECALL_munmap(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: munmap called by thread %i\n", GetActiveThreadID());

  uintptr_t Addr = RegFile->GetX<uintptr_t>(feature, 10);
  size_t Size = RegFile->GetX<size_t>(feature, 11);

  int rc = (int)mem->DeallocMem(Addr, Size);
  if(rc == -1){
    output->fatal(CALL_INFO, 11,
...
RevProc::ECALL_status_t RevProc::ECALL_mremap(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mremap called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_add_key(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: add_key called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_request_key(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: request_key called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_keyctl(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: keyctl called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_clone(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clone called by thread %i\n", GetActiveThreadID());
  auto rtval = ECALL_status_t::ERROR;
  uint64_t CloneArgsAddr = RegFile->GetX<uint64_t>(feature, 10);
  // size_t SizeOfCloneArgs = RegFile()->GetX<size_t>(feature, 11);
...
// TODO: See the reference clone() code above
//
// ECALL.buf.data() needs to be used instead of ECALL_buf.data().
// ECALL.bytesRead needs to be used instead of ECALL_bytesRead.
// This allows ECALL to be stored in a Hart-specific way and
// all fields to be initialized in a struct/class.
// Zero-terminated strings ARE NOT assumed -- use std::string as an
// arbitrary-size buffer, not a zero-terminated string except in
// APIs which use it, for which ECALL_LoadAndParseString should be
// used.
...
RevProc::ECALL_status_t RevProc::ECALL_clone(RevInst& inst){
  ...
  // std::shared_ptr<RevThread> ParentThread = ThreadTable.at(ActiveThreadIDs.at(HartToExec));

  // Create the child ctx
  // uint32_t ChildThreadID = SpawnThread();
  // std::shared_ptr<RevThread> ChildThread = ThreadTable.at(ChildThreadID);

  // TODO: Create a copy of Parents Memory Space
  // std::pair<uint64_t, uint64_t> ParentThreadMemPtrs = ParentThread->GetThreadMemInfo();
  // std::pair<uint64_t, uint64_t> ChildThreadMemPtrs = ChildThread->GetThreadMemInfo();
...
RevProc::ECALL_status_t RevProc::ECALL_execve(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: execve called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_mmap(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mmap called by thread %i\n", GetActiveThreadID());

  uint64_t addr = RegFile->GetX<uint64_t>(feature, 10);
  uint64_t size = RegFile->GetX<uint64_t>(feature, 11);
  // int prot = RegFile->GetX<int>(feature, 12);
  // int Flags = RegFile->GetX<int>(feature, 13);
  // int fd = RegFile->GetX<int>(feature, 14);
  // off_t offset = RegFile->GetX<off_t>(feature, 15);
...
RevProc::ECALL_status_t RevProc::ECALL_mmap(RevInst& inst){
  ...
  uint64_t addr = RegFile->GetX<uint64_t>(feature, 10);
  uint65_t size = RegFile->GetX<uint64_t>(feature, 11);
  // int prot = RegFile->GetX<int>(feature, 12);
  // int Flags = RegFile->GetX<int>(feature, 13);
  // int fd = RegFile->GetX<int>(feature, 14);
  // off_t offset = RegFile->GetX<off_t>(feature, 15);
...
RevProc::ECALL_status_t RevProc::ECALL_mincore(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mincore called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 233, rev_madvise(unsigned long start, size_t len, int behavior)
RevProc::ECALL_status_t RevProc::ECALL_madvise(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: madvise called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_remap_file_pages(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: remap_file_pages called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_mbind(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: mbind called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_get_mempolicy(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: get_mempolicy called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_set_mempolicy(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: set_mempolicy called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_migrate_pages(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: migrate_pages called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_move_pages(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: move_pages called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_rt_tgsigqueueinfo(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rt_tgsigqueueinfo called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 241, rev_perf_event_open(")
RevProc::ECALL_status_t RevProc::ECALL_perf_event_open(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: perf_event_open called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 242, rev_accept4(int, struct sockaddr  *, int  *, int)
RevProc::ECALL_status_t RevProc::ECALL_accept4(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: accept4 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_recvmmsg_time32(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: recvmmsg_time32 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_wait4(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: wait4 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_prlimit64(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: prlimit64 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_fanotify_init(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fanotify_init called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_fanotify_mark(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fanotify_mark called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_name_to_handle_at(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: name_to_handle_at called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_open_by_handle_at(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: open_by_handle_at called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_clock_adjtime(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clock_adjtime called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 267, rev_syncfs(int fd)
RevProc::ECALL_status_t RevProc::ECALL_syncfs(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: syncfs called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 268, rev_setns(int fd, int nstype)
RevProc::ECALL_status_t RevProc::ECALL_setns(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: setns called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_sendmmsg(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sendmmsg called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_process_vm_readv(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: process_vm_readv called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_process_vm_writev(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: process_vm_writev called by thread %i\n", GetActiveThreadID());
...
RevProc::ECALL_status_t RevProc::ECALL_kcmp(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: kcmp called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 273, rev_finit_module(int fd, const char  *uargs, int flags)
RevProc::ECALL_status_t RevProc::ECALL_finit_module(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: finit_module called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_sched_setattr(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_setattr called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_sched_getattr(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: sched_getattr called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_memfd_create(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: memfd_create called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 280, rev_bpf(int cmd, union bpf_attr *attr, unsigned int size)
RevProc::ECALL_status_t RevProc::ECALL_bpf(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: bpf called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_copy_file_range(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: copy_file_range called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_preadv2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: preadv2 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_pwritev2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pwritev2 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_pkey_mprotect(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pkey_mprotect called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_pkey_alloc(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pkey_alloc called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 290, rev_pkey_free(int pkey)
RevProc::ECALL_status_t RevProc::ECALL_pkey_free(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pkey_free called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_statx(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: statx called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_io_pgetevents(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_pgetevents called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_rseq(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: rseq called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_kexec_file_load(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: kexec_file_load called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_pidfd_send_signal(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pidfd_send_signal called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_io_uring_setup(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_uring_setup called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_io_uring_enter(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_uring_enter called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_io_uring_register(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: io_uring_register called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 428, rev_open_tree(int dfd, const char  *path, unsigned flags)
RevProc::ECALL_status_t RevProc::ECALL_open_tree(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: open_tree called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_move_mount(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: move_mount called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 430, rev_fsopen(const char  *fs_name, unsigned int flags)
RevProc::ECALL_status_t RevProc::ECALL_fsopen(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fsopen called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_fsconfig(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fsconfig called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_fsmount(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fsmount called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 433, rev_fspick(int dfd, const char  *path, unsigned int flags)
RevProc::ECALL_status_t RevProc::ECALL_fspick(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: fspick called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 434, rev_pidfd_open(pid_t pid, unsigned int flags)
RevProc::ECALL_status_t RevProc::ECALL_pidfd_open(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pidfd_open called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

#if USE_OLD_SYSCALLS

// clone3 not implemented

// 435, rev_clone3(struct clone_args  *uargs, size_t size)
RevProc::ECALL_status_t RevProc::ECALL_clone3(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clone3 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

#else

// TODO:  See reference clone() code above
//
// RegFile->GetX<type>(feature, register) and RegFile->SetX(feature, register, value)
// should be used. No casting is required becuase these functions handle all integer
// types automatically with no user-casts required.
//
// ECALL.buf.data() needs to be used instead of ECALL_buf.data().
// ECALL.bytesRead needs to be used instead of ECALL_bytesRead.
// This allows ECALL to be stored in a Hart-specific way and
...
RevProc::ECALL_status_t RevProc::ECALL_clone3(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: clone3 called by thread %i\n", GetActiveThreadID());
  // TODO: RegFile->GetX<cast_type>(feature, register) should be used
  // uint64_t CloneArgsAddr = RegFile->GetX<uint64_t>(feature, 10);
  // size_t SizeOfCloneArgs = RegFile()->GetX<size_t>(feature, 11);
  ...
  for( uint64_t bit=1; bit != 0; bit <<= 1 ){
    switch (args.flags & bit) {
      ...
      case CLONE_SIGHAND: /* Set if signal handlers shared */
        ...
        break;
      case CLONE_PIDFD: /* Set if a pidfd should be placed in the parent */
        // std::cout << "CLONE_ThreadIDFD is true" << std::endl;
        break;
      case CLONE_PTRACE: /* Set if tracing continues on the child */
        // std::cout << "CLONE_PTRACE is true" << std::endl;
        break;
...
RevProc::ECALL_status_t RevProc::ECALL_close_range(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: close_range called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_openat2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: openat2 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}

// 438, rev_pidfd_getfd(int pidfd, int fd, unsigned int flags)
RevProc::ECALL_status_t RevProc::ECALL_pidfd_getfd(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: pidfd_getfd called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_faccessat2(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: faccessat2 called by thread %i\n", GetActiveThreadID());
  return ECALL_status_t::SUCCESS;
}
...
RevProc::ECALL_status_t RevProc::ECALL_printf(RevInst& inst) {
  ...
  if('\0' != ECALL.buf[0]){
    ...
    //first time through the ECALL
    uint64_t addr = RegFile->GetX<uint64_t>(feature, 10);
    mem->ReadVal<char>(HartToExec, addr, &ECALL.buf[0], inst.hazard, REVMEM_FLAGS(0x00));
    rtval = ECALL_status_t::CONTINUE;
    DependencySet(HartToExec, 10, false);
  }
  ...
  if( rtval == ECALL_status_t::SUCCESS){
    ...
    // va_list args;
    // va_start(args, ECALL.string); // Normally, you'd use the last named argument here
    ...
    // int rc = vprintf(ECALL.string, args);
...
RevProc::ECALL_status_t RevProc::ECALL_pthread_create(RevInst& inst){
  ...
  uint64_t tidAddr = RegFile->GetX<uint64_t>( feature, 10 ); // Used to hold the new ThreadID
  // const pthread_attr_t *restrict attr = (const pthread_attr_t *)RegFile->GetX<uint64_t>(feature, 11);
  ...
  mem->WriteMem(feature->GetHart(), tidAddr, sizeof(NewTID), &NewTID, REVMEM_FLAGS(0x00));
  return ECALL_status_t::SUCCESS;
}

// 1001, int rev_pthread_join(pthread_t thread, void **retval);
RevProc::ECALL_status_t RevProc::ECALL_pthread_join(RevInst& inst){
  RevProc::ECALL_status_t rtval = ECALL_status_t::CONTINUE;
  ...
  AssignedThreads.at(HartToDecode)->SetWaitingToJoinTID(RegFile->GetX<uint64_t>(feature, 10));

  // // if retval is not null, store the return value of the thread in retval
  // void **retval = (void **)RegFile->GetX(feature, 11);
  // if( retval != NULL ){
  //   *retval = AssignedThreads.at(HartToDecode)->GetRegFile()->GetX<uint64_t>(feature, 10);
  // }
  //
  return rtval;
}
