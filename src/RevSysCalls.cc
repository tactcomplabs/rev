...
EcallStatus RevProc::ECALL_write(RevInst& inst){
  output->verbose(CALL_INFO, 2, 0, "ECALL: write called on Hart %" PRIu16 "by thread %" PRIu32 " on hart %" PRIu16 "\n", HartToExec, GetActiveThreadID(), HartToExec);
  auto fd = RegFile->GetX<int>(RevReg::a0);
...
EcallStatus RevProc::ECALL_exit(RevInst& inst){
  ...
  output->verbose(CALL_INFO, 0, 0,
                  "thread %" PRIu32 " on hart %" PRIu16 "exiting with status %" PRIu64 "\n",
...
EcallStatus RevProc::ECALL_brk(RevInst& inst){
  ...
  if( Addr > 0 && Addr > heapend ){
    ...
    output->fatal(CALL_INFO, 11,
                  "Out of memory / Unable to expand system break (brk) to Addr = 0x%" PRIx64 "\n", Addr);
  }
  return EcallStatus::SUCCESS;
}
