...
uint16_t RevProc::GetHartID() const {
  // TODO: (BEFORE MERGE) --- Potentially make this a bitset that is MAX HARTS long
...
void RevProc::MarkLoadComplete(const MemReq& req){
  ...
  if( it != LSQueue->end()){
    ...
    output->fatal(CALL_INFO, -1,
                  "Core %" PRIu16 "; Hart %" PRIu16 "; Cannot find outstanding load for reg %u from address %" PRIx64 "\n",
...
bool RevProc::ClockTick( SST::Cycle_t currentCycle ){
  ...
  if( AssignedThreads.size() > Harts.size() ){
    output->fatal(CALL_INFO, 99, "Proc %" PRIu16 " has become oversubscribed which is not currently supported\n");
  }
  ...
  for( size_t HartID=0; HartID<Harts.size(); HartID++ ){
    if( HartHasThread(HartID) ){
      HART_CTS[HartID] = (ThreadOnHart(HartID)->GetRegFile()->cost == 0);
    } else {
      HART_CTS[HartID] = false;
    }
  }

  // TODO: (BEFORE MERGE) --- Either leave this or switch to bitset that's length is always "MAX HARTS" and then use HART_CTS.any()
  if( std::any_of(HART_CTS.begin(), HART_CTS.end(), [](bool x){ return true; }) && (!Halted)) {
    ...
    RegFile = ThreadOnHart(HartToDecode)->GetRegFile();
  ...
  if( ( (HartToExec != _REV_INVALID_HART_ID_) && !RegFile->GetTrigger()) && !Halted && HART_CTE[HartToExec] && HartHasThread(HartToExec)){
    ...
    if( ExecPC != _PAN_FWARE_JUMP_ ){
      ...
      if( !Ext->Execute(EToE.second, Pipeline.back().second, HartToExec, RegFile) ){
        output->fatal(CALL_INFO, -1,
                      "Core %" PRIu32 "; Hart %" PRIu32 "; Thread %" PRIu32 "; Error: failed to execute instruction at PC=%" PRIx64 ".",
                      id, HartToExec, ThreadOnHart(HartToExec)->GetThreadID(), ExecPC );
      }
      ...
      if( (RegFile->RV64_SCAUSE == EXCEPTION_CAUSE::ECALL_USER_MODE) ||
        ...
        // Ecall found
        output->verbose(CALL_INFO, 6, 0,
                        "Core %" PRIu16 "; Hart %" PRIu16 "; Thread %" PRIu32 " - Exception Raised: ECALL with code = %" PRIu64 "\n",
    ...
    output->verbose(CALL_INFO, 9, 0,
                    "Core %" PRIu16 " ; Hart %" PRIu16 "; No available hart to exec PC= 0x%" PRIx64 "\n",
                    id, HartToDecode, ExecPC);
    rtn = true;
  ...
  if(!Pipeline.empty() &&
    ...
    if(Pipeline.front().second.cost == 0){ // &&
      ...
      output->verbose(CALL_INFO, 6, 0,
                      "Core %" PRIu16 "; Hart %" PRIu16 "; Thread %" PRIu32 "; Retiring PC= 0x%" PRIx64 "\n",
                      id, HartID, ThreadOnHart(HartID)->GetThreadID(), ExecPC);
      Retired++;
      ...
      ThreadOnHart(HartID)->GetRegFile()->SetCost(0);
    }else{
  ...
  if( (GetPC() == _PAN_FWARE_JUMP_) || (GetPC() == 0x00ull) ){
    ...
    if( GetPC() == _PAN_FWARE_JUMP_ ){
      ...
      if( ThreadCanBeRemoved(ThreadOnHart(HartToExec)->GetThreadID())){
        ThreadOnHart(HartToExec)->SetState(ThreadState::DONE);
        HART_CTE[HartToExec] = false;
        HART_CTS[HartToExec] = false;
        ThreadsThatChangedState.emplace(ThreadOnHart(HartToDecode));
      }
    }

    if( HartToExec != _REV_INVALID_HART_ID_ && HartHasThread(HartToExec) && ThreadCanBeRemoved(ThreadOnHart(HartToExec)->GetThreadID()) ){
      output->verbose(CALL_INFO, 2, 0,
                      "Thread %" PRIu32 " is done\n", ThreadOnHart(HartToExec)->GetThreadID());
      HART_CTE[HartToExec] = false;
      HART_CTS[HartToExec] = false;
      ThreadOnHart(HartToExec)->SetState(ThreadState::DONE);
      ThreadsThatChangedState.emplace(ThreadOnHart(HartToDecode));
...
// TODO: (BEFORE MERGE) Potentially add a faster way to find available Harts
...
void RevProc::AssignThread(const uint32_t& ThreadID){
  ...
  // If we couldn't find a hart without a thread, this is a bug
  if( !ThreadAssigned ){
...
uint32_t RevProc::GetActiveThreadID(){
  uint32_t ThreadID = _INVALID_TID_;
  if( (HartToExec != _REV_INVALID_HART_ID_) && HartHasThread(HartToExec) ){
    ThreadID = ThreadOnHart(HartToExec)->GetThreadID();
  } else {
    output->fatal(CALL_INFO, -1, "Attempted to get active thread ID but HartToExec (%" PRIu32 ") has no thread assigned to it.\n", HartToExec);
  }
